...
var INIT_VECTOR = [ROUNDS + 2 + 4*256]uint32{
...
func (bf *Blowfish) f(x uint32) uint32 {
	//x разбивается на четыре 8-битных квартала a, b, c и d, используя побитовые операции и сдвиги вправо.
	a := uint8(x & 0xff)
	x >>= 8
	b := uint8(x & 0xff)
	x >>= 8
	...
	//Складывается по модулю 2^32 значения из первого и второго S-бокса, используя d и c в качестве индексов,
	//и присваивается результат переменной h.
	h := bf.S[0][d] + bf.S[1][c]
	h ^= bf.S[2][b]
	return h + bf.S[3][a]
}

// EncryptBlock принимает два указателя на 32-битные целые числа, L и R, которые представляют левую и правую
// половины 64-битного блока данных, подлежащих шифрованию. Функция также имеет доступ к структуре Blowfish,
// которая содержит массив P из 18 подключей и четыре массива S из 256 элементов каждый.
func (bf *Blowfish) EncryptBlock(l, r *uint32) {
	//Сохраняются значения L и R в локальных переменных L и R.
	L := *l
	R := *r

	//Выполняется 16 раундов шифрования, увеличивая счетчик i на 2 на каждой итерации. Каждый раунд состоит
	//из четырех операций
	...
	//После 16-го раунда отменяется последний обмен и XOR L с P[18] и R с P[17] (выходное отбеливание).
	//Присваиваются значения L и R обратно указателям *l и *r, меняя их местами.
...
func (bf *Blowfish) Encrypt(text []byte, bar *progressbar.ProgressBar) []byte {
	...
	for i := 0; i < len(blocks); i += 2 {
		...
		bar.Set(int((float64(i) / float64(len(blocks))) * float64(10)))
	}
	bar.Set(100)
...
func (bf *Blowfish) Decrypt(cipherText []byte, bar *progressbar.ProgressBar) []byte {
	bar = progressbar.Default(100)
	blocks := []uint32{}
...
// NewBlowfish создает и возвращает новый экземпляр структуры Blowfish, которая содержит подключи для
// шифрования и расшифрования данных. Для этого она инициализирует и модифицирует массивы P и S-блоков, используя
// заданный ключ и константы
func NewBlowfish(key []byte) *Blowfish {
	bf := &Blowfish{}

	keyLen := len(key)

	//Инициализируется четыре S-блока по 256 32-битных элементов каждый.
	for i := 0; i < 4; i++ {
		for j := 0; j < 256; j++ {
	...
	//Складывается по модулю 2(XOR) каждый элемент массива P с 32-битным фрагментом ключа. Если ключ короче 72 байт,
	//он повторяется циклически
	...
	//Шифруется нулевой 64-битный блок, используя текущие значения массива P и S-блоков.
	//Затем заменяются первые два элемента массива P результатом шифрования, это повторяется
	//для всех остальных элементов массива P, используя предыдущий зашифрованный блок в качестве входных данных
	...
	//Повторяются предыдущие два шага для всех элементов S-блоков, используя последний зашифрованный блок в
	...
	//Возвращает указатель на структуру Blowfish, содержащую сгенерированные массивы P и S-блоков
	return bf
}
